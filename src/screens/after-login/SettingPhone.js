...
const SettingPhone = ({ navigation }) => {
  const request = useRequest();
  const authState = useSelector((state) => state.auth);
  const changePhoneValidationScheme = yup.object().shape({
    phone: yup.string()
  ...
  const formik = useFormik({
    validationSchema: changePhoneValidationScheme,
    initialValues: {
      phone: authState._user.data?.phone,
    },
          ...
          <View style={[styles.bgWhite]}>
            ...
            <View style={[styles.mb10]}>
              <View style={[styles.row]}>
                ...
                <TouchableHighlight onPress={() => navigation.navigate('SettingPhone')} style={{ width: '33.333333%', justifyContent: 'center', alignItems: 'center', backgroundColor: '#00c20b', borderColor: '#00c20b', borderWidth: 1 }} underlayColor="white">
                  <Text style={{ color: '#fff', padding: 10, textAlign: 'center' }}>Change Phone</Text>
                </TouchableHighlight>
                <TouchableHighlight onPress={() => navigation.navigate('SettingEmail')} style={{ width: '33.333333%', justifyContent: 'center', alignItems: 'center', backgroundColor: '#ddd', color: '#333', borderColor: '#999', borderWidth: 1 }} underlayColor="white">
                  <Text style={{ padding: 10, textAlign: 'center' }}>Change Email</Text>
                </TouchableHighlight>
              </View>
            </View>
            {/* Form Change Password */}
            <Text style={{ fontSize: 20, marginBottom: 5, color: '#333' }}>Change Phone</Text>
            <Text style={{color: '#333'}}>Phone</Text>
            <TextInput
